...
class StringManipulator
{
    ...
    public static function slugfyFileName(string $fullString, $options = array()): string
    {
        ...
        if ($options['transliterate']) {
            $stringBody = str_replace(array_keys($char_map), $char_map, $stringBody);
        }

        $stringBody = preg_replace('/[^\p{L}\p{Nd}]+/u', $options['delimiter'], $stringBody);

        $stringBody = preg_replace('/(' . preg_quote($options['delimiter'], '/') . '){2,}/', '$1', $stringBody);

        $stringBody = mb_substr($stringBody, 0, $options['limit'] ?? mb_strlen($stringBody, 'UTF-8'), 'UTF-8');

        $stringBody = trim($stringBody, $options['delimiter']);

        return ($options['lowercase'] ? mb_strtolower($stringBody, 'UTF-8') : $stringBody) . $stringSufix;
    }
}